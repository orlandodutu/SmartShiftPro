...
const ROLE_THEME: Record<Ruolo, { bg: string; border: string; accent: string; avatar: string }> = {
  OSS:        { bg: "bg-blue-950/40",    border: "border-blue-800/50",    accent: "text-blue-300",    avatar: "bg-blue-900/60 text-blue-300"    },
  INFERMIERA: { bg: "bg-emerald-950/40", border: "border-emerald-800/50", accent: "text-emerald-300", avatar: "bg-emerald-900/60 text-emerald-300" },
  AUSILIARIO: { bg: "bg-amber-950/40",   border: "border-amber-800/50",   accent: "text-amber-300",   avatar: "bg-amber-900/60 text-amber-300"   },
  DEV:        { bg: "bg-indigo-950/40",  border: "border-indigo-800/50",  accent: "text-indigo-300",  avatar: "bg-indigo-900/60 text-indigo-300"  },
  CAPOSALA:   { bg: "bg-yellow-950/40",  border: "border-yellow-800/50",  accent: "text-yellow-300",  avatar: "bg-yellow-900/60 text-yellow-300"  },
};
...
export default function Dashboard() {
  ...
  const theme = ROLE_THEME[role] ?? ROLE_THEME.DEV;
  ...
  const handleResetCompleto = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: "Sistema azzerato — turni e statistiche eliminati" });
        setResetOpen(false);
  ...
  const fetchData = useCallback(async () => {
    if (!user) return;
    try {
      const [statsRes, dipRes, turniRes] = await Promise.all([
        fetch("/flask-api/api/statistiche", { credentials: "include" }),
        fetch("/flask-api/api/dipendenti", { credentials: "include" }),
        fetch(`/flask-api/api/turni?mese=${mese}&anno=${anno}`, { credentials: "include" }),
      ]);
  ...
  /* Fetch shifts for selected staff */
  useEffect(() => {
      ...
      if (res.ok) {
        ...
        if (riadattati > 0 && tipiRimossi.length > 0) {
          toast({
            ...
            description: `${riadattati} turno/i futuri di tipo ${tipiRimossi.join(", ")} riadattati automaticamente.`,
          });
        } else {
      ...
      <div className={`${theme.bg} border-b ${theme.border} px-6 md:px-10 py-8`}>
        <div className="max-w-7xl mx-auto flex items-center gap-5">
          ...
          <div className="flex-1">
            ...
            <p className={`text-sm font-medium ${theme.accent}`}>Pannello di controllo — SmartShift Pro</p>
          </div>
              ...
              <CardContent>
                {loading ? (
                  ...
                  <div className="space-y-3">
                    {/* Turni oggi */}
                    {turniOggi.length === 0 ? (
                      <p className="text-sm text-muted-foreground py-3 text-center">Nessun turno generato per oggi</p>
                    ) : (
                      <div className="space-y-1.5">
                        {(["MATTINO","POMERIGGIO","NOTTE"] as const).map((tipo) => {
                          const count = turniOggi.filter((t) => t.tipo === tipo).length;
                          if (!count) return null;
                          return (
                            <div key={tipo} className="flex items-center justify-between px-3 py-2 rounded-xl bg-white/3 border border-white/8">
                              <ShiftBadge type={tipo} />
                    ...
                    {assentiOggi.length > 0 && (
                      <div className="rounded-xl bg-red-500/8 border border-red-500/20 p-3">
                        <p className="text-xs font-bold text-red-400 mb-2 uppercase tracking-wide">Assenti oggi</p>
                            ...
                            return (
                              <div key={a.id} className="flex items-center gap-2 text-xs">
                                {a.tipo === "MALATTIA" ? (
                                  ...
                                  <Palmtree className="h-3 w-3 text-emerald-400 shrink-0" />
                                )}
                                <span className="text-foreground font-medium">{dip?.nome ?? "—"}</span>
                                <span className={`text-[10px] ${a.tipo === "MALATTIA" ? "text-red-400" : "text-emerald-400"}`}>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-white/5 hover:bg-transparent">
                      ...
                      <TableHead className="text-muted-foreground text-xs">Stato oggi</TableHead>
                      <TableHead className="text-right text-muted-foreground text-xs">Ore</TableHead>
                      <TableHead className="text-right pr-4 text-muted-foreground text-xs">Notti</TableHead>
                  ...
                  <TableBody>
                    {stats.filter((d) => !d.is_admin && d.ruolo !== "CAPOSALA").sort((a, b) => b.ore_totali - a.ore_totali).map((dip) => {
                      ...
                      return (
                        <TableRow
                          key={dip.id}
                          className="border-white/5 transition-colors hover:bg-white/3 cursor-pointer"
                          onClick={() => openProfile(dip)}
                        >
                          ...
                          <TableCell>
                            {absence ? (
                              ...
                              <ShiftBadge type={turnoOggi.tipo} />
                            ) : (
        ...
        <DialogContent className="glass-strong border-white/10 max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          {profileTab === "assenze" && (
            <div className="space-y-5 mt-2">
              ...
              <div className="space-y-2">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide">Storico — {profileTarget?.nome}</p>
                {assenzeLoading ? (
                    ...
                    {assenze.map((a) => {
                      ...
                      return (
                        <div key={a.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${
                          active
                            ? a.tipo === "MALATTIA" ? "bg-red-500/8 border-red-500/20" : "bg-emerald-500/8 border-emerald-500/20"
                            : "bg-white/3 border-white/8"
                        }`}>
                          <div className={`p-2 rounded-lg shrink-0 ${a.tipo === "MALATTIA" ? "bg-red-500/15 text-red-400" : "bg-emerald-500/15 text-emerald-400"}`}>
            ...
            <div className="space-y-4">
              <div className="rounded-xl bg-red-500/8 border border-red-500/20 p-4 space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Questa azione è <strong className="text-red-400">irreversibile</strong>. Verranno eliminati tutti i turni e le assenze associate.
                </p>
              </div>
          ...
          <div className="space-y-4">
            <div className="rounded-xl bg-red-500/8 border border-red-500/20 p-4 space-y-2">
              ...
              <ul className="text-xs text-muted-foreground space-y-0.5 ml-3">
                ...
                <li>• Tutto l'archivio storico</li>
                <li>• Tutte le assenze registrate</li>
                <li>• Contatori ore, notti, ferie e malattia</li>
              </ul>
              <p className="text-xs text-amber-400/80 mt-2 font-medium">Gli account e le preferenze vengono mantenuti.</p>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs text-muted-foreground">
                Digita <span className="font-bold text-red-400">RESET</span> per sbloccare
              </Label>
              <Input
          ...
          <form onSubmit={handleAddUser} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={newUser.ruolo} onValueChange={(v) => setNewUser({ ...newUser, ruolo: v as Ruolo })}>
                ...
                <SelectContent>
                  {(["OSS", "INFERMIERA", "AUSILIARIO"] as Ruolo[]).map((r) => (
